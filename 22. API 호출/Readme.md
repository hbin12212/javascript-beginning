...
커피숍에서는 우리가 커피숍에 들어가서 직접 원두를 갈고 커피를 만들어 먹지 않고, 커피숍의 메뉴판을 통해 원하는 커피를 선택하고 직원에게 주문을 하게 됩니다.
...
1. 웹브라우저가 서버에게 원하는 데이터를 요청합니다.
2. 서버는 데이터베이스에서 브라우저로부터 요청받은 데이터를 찾고 꺼냅니다.
3. 서버가 웹브라우저에게 데이터를 전달합니다.

이 과정에 나오는 웹브라우저(클라이언트)는 커피를 주문하는 우리, 서버는 점원, 데이터베이스는 창고 그리고 데이터는 커피 라고 할 수 있습니다.

커피숍에서 커피를 시키는 과정과 마찬가지로 클라이언트는 원하는 데이터가 있을 경우, 다른 데이터베이스에 직접 접근하지 않고 원하는 데이터를 `요청` 합니다.

그 다음, 점원이 창고에서 알맞은 재료를 찾아 우리가 주문한 커피를 제작하듯이 서버 또한 클라이언트가 요청한 데이터를 데이터베이스에서 찾고 꺼냅니다.

마지막으로 점원이 완성된 커피를 우리에게 전달하는 것처럼, 서버는 요청받은 데이터를 클라이언트에게 전달하는 것이죠.

### 그래서 API란?

이제 클라이언트와 서버의 통신은 완벽하게 이해하셨을 겁니다.

API는 이러한 서버와 클라이언트의 통신에 약속을 더한 것 뿐입니다.

다시말해, API는 서버와 클라이언트가 통신하는 과정 속에서 어떠한 방식으로 데이터를 요청하고 전달하겠다는 약속이라고 할 수 있습니다.

그럼 이제는 API를 호출하는 방법 즉, 서버에게 데이터를 요청하고 전달받는 과정을 자바스크립트로 어떻게 작성해야 하는지, 그리고 API 호출 시 정해져있는 약속들은 어떤 것들이 있는지 알아보도록 하겠습니다.

## API 호출

지금부터는 특정 서버에 데이터를 요청하고 받아보는 API를 실제로 호출해보겠습니다.
...
다시 돌아와서 위의 json 형식의 데이터들은 API 호출의 응답 결과들 입니다.

즉, 우리가 이 API를 이용해 호출을 하면 서버는 페이지에 보이는 것과 같은 json 데이터들을 전달해주게 된다는 뜻 입니다.

그럼 이제 실제로 API를 호출해보겠습니다.

우리는 jsonplaceholder에서 제공해주는 API url 중 하나인, https://jsonplaceholder.typicode.com/posts/1 를 이용해서 API를 호출해보겠습니다.

```js
fetch("https://jsonplaceholder.typicode.com/posts/1").then((response) => console.log(response));
```

자바스크립트에서는 위 코드와 같이 내장함수 `fetch`를 이용해 API를 호출할 수 있습니다.
...
내장함수 fetch는 위와 같이 파라미터로 url을 사용하고, then을 통해 호출 결과값을 받을 수 있고, catch 를 통해 에러처리를 할 수 있습니다.

여기서 알 수 있듯이, API는 url 형식으로 서버에 요청을 보낸다는 약속을 갖고 있습니다.
...
이렇게 응답 객체의 json 메서드를 이용하면, 응답 결과 데이터를 json 형태로 변환하여 요청에 대한 순수한 응답 데이터만 얻을 수 있습니다.
...
API 호출에 대한 응답이 오게되면 아래 작성된, 응답을 json으로 변환하는 코드가 실행되고, 마찬가지로 응답을 json으로 변환하는 과정이 완료되기 전까지 결과를 출력하는 코드가 실행되지 않습니다.
